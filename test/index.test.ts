...
test.only('bbox', () => {
  expect(sm.bbox(0, 0, 0, true, 'WGS84')).toStrictEqual(MAX_EXTENT_WGS84);
  expect(sm.bbox(0, 0, 1, true, 'WGS84')).toStrictEqual([-180, -85.05112877980659, 0, 0]);
});

test('xyz', () => {
  expect(sm.xyz([-180,-85.05112877980659,180,85.0511287798066],0,true,'WGS84')).toStrictEqual({minX:0,minY:0,maxX:0, maxY:0});
  expect(sm.xyz([-180,-85.05112877980659,0,0],1,true,'WGS84')).toStrictEqual({minX:0,minY:0,maxX:0, maxY:0});  
});
...
test('convert', () => {
  expect(sm.convert(MAX_EXTENT_WGS84 as any, '900913')).toStrictEqual(MAX_EXTENT_MERC);
  expect(sm.convert(MAX_EXTENT_MERC as any, 'WGS84')).toStrictEqual(MAX_EXTENT_WGS84);
});

test('extents', () => {
  expect(sm.convert([-240, -90, 240, 90], '900913')).toStrictEqual(MAX_EXTENT_MERC);
...
test('ll', () => {
  expect(sm.ll([200,200], 9)).toStrictEqual([-179.45068359375, 85.00351401304403]);
  expect(sm.ll([200,200], 8.6574)).toStrictEqual([-179.3034449476476, 84.99067388699072]);
});
...
test('px', () => {
  ...
  expect(sm.px([-179,85], 8.6574)).toStrictEqual([287.12734093961626, 169.30444219392666]);
